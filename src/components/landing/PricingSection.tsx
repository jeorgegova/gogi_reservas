...
export function PricingSection() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting || hasNudged || reducedMotion || scroller.scrollLeft > 8) return;

        hasNudged = true;
        const firstCard = scroller.firstElementChild as HTMLElement | null;
        const nudgeDistance = Math.min(140, Math.max(96, (firstCard?.offsetWidth || window.innerWidth) * 0.38));

        nudgeTween = gsap.timeline({ delay: 0.35 })
          .to(scroller, {
            scrollLeft: nudgeDistance,
            duration: 0.55,
            ease: 'power2.out',
          })
          .to(scroller, {
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-8 md:mb-24">
          ...
          <h2 className="text-2xl md:text-5xl font-semibold text-slate-900 tracking-tight mt-4 mb-3 md:mb-6">
